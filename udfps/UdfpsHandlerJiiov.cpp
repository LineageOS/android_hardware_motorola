...
// Unlike the goodix/egistec handlers, Jiiov exposes no
// vendor.*.hardware.fingerprint HIDL/AIDL service to poke for FOD events, so
// this handler talks to anc.hal.so directly. The library and its entry points
// are resolved once in init() and cached, rather than dlopen()'d per touch.
namespace {
