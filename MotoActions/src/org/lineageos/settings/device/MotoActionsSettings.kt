...
class MotoActionsSettings(
    ...
    private val prefListener = OnSharedPreferenceChangeListener { sharedPrefs, key ->
        ...
        when (key) {
            GESTURE_CHOP_CHOP_KEY ->
                chopChopEnabled =
                    sharedPrefs.getBoolean(GESTURE_CHOP_CHOP_KEY, true)
            GESTURE_IR_WAKEUP_KEY ->
                irWakeUpEnabled =
                    sharedPrefs.getBoolean(GESTURE_IR_WAKEUP_KEY, true)
            GESTURE_PICK_UP_KEY ->
                pickUpGestureEnabled =
                    sharedPrefs.getBoolean(GESTURE_PICK_UP_KEY, true)
            GESTURE_POCKET_KEY ->
                pocketGestureEnabled =
                    sharedPrefs.getBoolean(GESTURE_POCKET_KEY, true)
            GESTURE_IR_SILENCER_KEY ->
                irSilencerEnabled =
                    sharedPrefs.getBoolean(GESTURE_IR_SILENCER_KEY, false)
            GESTURE_FLIP_TO_MUTE_KEY ->
                flipToMuteEnabled =
                    sharedPrefs.getBoolean(GESTURE_FLIP_TO_MUTE_KEY, false)
            GESTURE_LIFT_TO_SILENCE_KEY ->
                liftToSilenceEnabled =
                    sharedPrefs.getBoolean(GESTURE_LIFT_TO_SILENCE_KEY, false)
            else -> updated = false
        }
    ...
    fun isIrWakeupEnabled(): Boolean =
        isDozeEnabled() && !isAlwaysOnEnabled() && irWakeUpEnabled

    fun isPickUpEnabled(): Boolean =
        isDozeEnabled() && !isAlwaysOnEnabled() && pickUpGestureEnabled
